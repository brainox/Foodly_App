            ...
            Auth.auth().createUser(withEmail: email, password: password) { authResult, error in
                if let error = error, authResult != nil {
                    ...
                    Firestore.firestore().collection("users").document(docId!).setData(
                        ["email": email, "fullName": fullName, "address": "Please update your address in the profile page", "phoneNumber": "Enter Phone Number"]) { (error) in
                        if error != nil {
                            HUD.hide()
...
//                                self.profileViewModel.createUserProfile()
                                self.navigateToHome()
                            }
